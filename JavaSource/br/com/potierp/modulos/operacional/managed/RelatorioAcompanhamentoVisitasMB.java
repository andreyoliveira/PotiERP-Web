 ...
 *
 */
public class RelatorioAcompanhamentoVisitasMB extends BaseMB{
	
	private static Logger log = Logger.getLogger(RelatorioAcompanhamentoVisitasMB.class);
	...
	public void doGerar() {
		try {
			if(isResponsavelSelecionado()){
					...
					if(rel != null) {
						super.registraArquivoParaDownload(rel, "Acompanhamento de Visitas", "pdf", PotiErpProperties.getInstance().getReportPath());
					} 
			}else{
	...
	private byte[] getRelatorio(final List<ProgramacaoVisita> listaProgramacaoVisita) throws PotiErpException {
		return operacionalModulo.getRelatorioAcompanhamentoVisita(listaProgramacaoVisita, responsavel, mesSelecionado);
	}
	...
	public String doRelatorioAcompanhamentoVisitas() {
		doCarregarComboMeses();
		return NavigationEnum.RELATORIO_ACOMPANHAMENTO_VISITAS.getValor();
	}
	...
	public boolean isConsultarRelatorioAcompanhamentoVisitas() {
		return isConsultar();
	}
